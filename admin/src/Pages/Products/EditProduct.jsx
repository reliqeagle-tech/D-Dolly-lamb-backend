...
import {
    ...
    TbArrowLeft, TbCloudUpload, TbTrash
} from "react-icons/tb";
...
const Skeleton = () => (
    <div style={{ minHeight: '100vh', background: B.bg }}>
        ...
        <div style={{ position: 'sticky', top: 0, zIndex: 40, background: B.surfaceCard, borderBottom: `1px solid ${B.border}`, height: 60, display: 'flex', alignItems: 'center', padding: '0 24px', gap: 12 }}>
            {[36, 1, 36].map((w, i) => (
                <div key={i} style={{ width: w, height: i === 1 ? 20 : 36, borderRadius: i === 1 ? 2 : 10, background: B.surface2, animation: 'skP 1.4s ease-in-out infinite' }} />
            ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, padding: '18px 24px', maxWidth: 1400 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {[280, 220, 200, 260].map((h, i) => (
                    <div key={i} style={{ height: h, borderRadius: 14, background: B.surfaceCard, border: `1px solid ${B.border}`, animation: 'skP 1.4s ease-in-out infinite' }} />
                ))}
            </div>
...
const UpdateProduct = ({ token }) => {
        ...
        const fetchProduct = async () => {
            ...
            try {
                ...
                if (res.data.success) {
                    ...
                    /* Colors */
                    if (p.color?.length) {
                        setColors(p.color.map(c => typeof c === 'string' ? { name: c, hex: '#808080' } : { name: c.name || c, hex: c.hex || '#808080' }));
                    }
                    ...
                    if (p.sizes?.length) {
                        ...
                        if (isStd) {
                            ...
                            p.sizes.forEach(s => {
                                ...
                                en.push(k);
                                const hasCustom = s?.useCustomPrice === true;
                                obj[k] = {
                                    multiplier: s?.priceMultiplier || 1,
                                    stock: s?.stock ?? 0,
                                    customPrice: s?.customPrice > 0 ? String(s.customPrice) : '',
                                    useCustomPrice: hasCustom,
                                };
                            });
                            setEnabledSz(en);
                            setStdSizes(obj);
                            /* Detect pricing mode from existing data */
                            const anyCustom = p.sizes.some(s => s?.useCustomPrice === true);
                            setPricingMode(anyCustom ? "custom" : "multiplier");
                        } else {
                            setSizeType('inch');
                            setInchSizes(p.sizes.map(s => ({
                                ...
                                useCustomPrice: s?.useCustomPrice || false,
                            })));
                        }
                    }
                } else {
    ...
    const addFilesToSlots = (files) => {
        ...
        setSlots(prev => {
            ...
            for (const file of files) {
                const ei = next.findIndex(s => !s.existing && !s.newFile);
                if (ei === -1) break;
                next[ei] = { existing: null, newFile: file };
                added++;
            }
            return next;
        });
        requestAnimationFrame(() => {
            ...
            else toast.info('All slots full');
        });
    };
    ...
    ════════════════════════════════════════════════════════ */
    const addColor = () => {
        if (!newCName.trim()) return toast.error('Enter color name');
        const c = { name: newCName.trim(), hex: newCHex || '#808080' };
    ...
    const setSzF = (k, f, v) => setStdSizes(p => ({
        ...
        [k]: {
            ...p[k],
            [f]: f === 'stock' ? parseInt(v) || 0 : f === 'multiplier' ? parseFloat(v) || 1 : v
        }
    }));
    ...
    const fmtSizes = () => sizeType === 'standard'
        ? enabledSz.map(k => ({
            size: k,
            priceMultiplier: stdSizes[k].multiplier,
            stock: stdSizes[k].stock,
            customPrice: stdSizes[k].customPrice,
            useCustomPrice: stdSizes[k].useCustomPrice,
        }))
        : inchSizes.map(s => ({
            size: s.size,
    ...
    const onSubmit = async (e) => {
        ...
        if (!description.trim()) return toast.error('Short description is required');
                    ...
                    <div>
                        ...
                        <Card icon={<TbPhoto size={17} />} title="Product Images" subtitle={`${totalImages}/10 · ${existingCount} existing · ${newFilesCount} new`}
                            ...
                            <div ref={dzRef} style={{
                                ...
                                {!dragging && (
                                    <input type="file" accept="image/*" multiple style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0, cursor: 'pointer', fontSize: 0 }}
                                            ...
                                            {hasImg ? (
                                                <>
                                                    ...
                                                    <div style={{ position: 'absolute', inset: 0, background: 'rgba(28,43,58,0.60)', opacity: isHovered ? 1 : 0, transition: 'opacity .18s', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, zIndex: 20 }}
                                                        ...
                                                        <button type="button" style={{ background: B.red.bg, color: B.red.text, fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 7, border: `1px solid ${B.red.border}`, cursor: 'pointer' }}
                                                ...
                                                <>
                                                    ...
                                                    <input type="file" accept="image/*" style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', fontSize: 0 }}
                                                        onChange={e => { if (e.target.files[0]) setSlotFile(i, e.target.files[0]); e.target.value = ''; }} />
                                                </>
                                            )}
                                        </div>
                                        ...
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: B.surfaceCard, border: `1px solid ${B.border}`, borderRadius: 11, transition: 'border-color .15s' }}
                                            ...
                                            <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                                                <input type="text" value={c.name} onChange={e => edColor(i, 'name', e.target.value)} placeholder="Color name"
                                                    style={{ fontSize: 13, fontWeight: 600, color: B.navy, background: 'transparent', border: 'none', borderBottom: `1px solid ${B.border}`, outline: 'none', padding: '3px 0', transition: 'border-color .15s' }}
                                                    onFocus={e => e.target.style.borderColor = B.greenBdr} onBlur={e => e.target.style.borderColor = B.border} />
                                                <input type="text" value={c.hex} onChange={e => edColor(i, 'hex', e.target.value)} placeholder="#000000"
                                                    style={{ fontSize: 12, color: B.navySoft, background: 'transparent', border: 'none', borderBottom: `1px solid ${B.border}`, outline: 'none', padding: '3px 0', fontFamily: 'monospace', transition: 'border-color .15s' }}
                                                    onFocus={e => e.target.style.borderColor = B.greenBdr} onBlur={e => e.target.style.borderColor = B.border} />
                                            </div>
                        ...
                        <Card icon={<TbRuler size={17} />} title="Sizes & Inventory" subtitle="Manage sizes, stock & pricing per size"
                            ...
                            <Field label="Base Price ($)" required>
                                <input style={inpStyle(price && (isNaN(+price) || +price <= 0))} type="number" placeholder="e.g. 4999" min="0" step="0.01"
                                    value={price} onChange={e => setPrice(e.target.value)} onFocus={focG} onBlur={blrB(price && (isNaN(+price) || +price <= 0))} />
                                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginTop: 8, padding: 10, background: B.blue.bg, border: `1px solid ${B.blue.border}`, borderRadius: 9 }}>
                                    ...
                                    <span style={{ color: B.blue.text, fontSize: 12 }}>
                                        Base price × multiplier = size's selling price. XL at 1.2× = ${price ? (parseFloat(price) * 1.2).toFixed(2) : '—'}. Enable Custom Price per size for fixed pricing.
                                    </span>
                                </div>
                            </Field>
                                ...
                                <div style={{ display: 'flex', gap: 10 }}>
                                    ...
                                    <button type="button"
                                        onClick={() => {
                                            ...
                                            Object.keys(updated).forEach(s => { updated[s].useCustomPrice = false; });
                                            setStdSizes(updated);
                                        }}
                                        style={{
                                            padding: '10px 18px', borderRadius: 10, fontWeight: 700, cursor: 'pointer', transition: '.2s',
                                            border: `1px solid ${pricingMode === "multiplier" ? B.green : B.border}`,
                                            background: pricingMode === "multiplier" ? B.green : B.surfaceCard,
                                            color: pricingMode === "multiplier" ? '#fff' : B.navy,
                                        }}>
                                        Multiplier Based
                                    </button>
                                    {/* Custom Price */}
                                    <button type="button"
                                        onClick={() => {
                                            ...
                                            const updated = { ...stdSizes };
                                            Object.keys(updated).forEach(s => { updated[s].useCustomPrice = true; });
                                            setStdSizes(updated);
                                        }}
                                        style={{
                                            padding: '10px 18px', borderRadius: 10, fontWeight: 700, cursor: 'pointer', transition: '.2s',
                                            border: `1px solid ${pricingMode === "custom" ? B.green : B.border}`,
                                            background: pricingMode === "custom" ? B.green : B.surfaceCard,
                                            color: pricingMode === "custom" ? '#fff' : B.navy,
                                        }}>
                                        Custom Price
                                    </button>
                                </div>
                                <p style={{ marginTop: 10, fontSize: 12, color: B.navyGhost }}>
                            ...
                            {sizeType === 'standard' && (
                                <>
                                                            ...
                                                            {pricingMode === "custom" ? (
                                                                <div>
                                                                    ...
                                                                    <input type="number" step="0.01" min="0" value={d.customPrice}
                                                                        ...
                                                                        onFocus={focG} onBlur={blrB()} />
                                                                    <span style={{ color: B.green, fontSize: 10.5, fontWeight: 700, marginTop: 3, display: 'block' }}>
                                                                        $ {d.customPrice || '—'}
                                                                    </span>
                                                                </div>
                                                            ) : (
                                                                <div>
                                                                    ...
                                                                    <input type="number" step="0.05" min="0.5" max="3" value={d.multiplier}
                                                                        ...
                                                                        style={{ ...inpStyle(), padding: '6px 10px', fontSize: 12 }}
                                                                        onFocus={focG} onBlur={blrB()} />
                                                                    <span style={{ color: B.emerald.text, fontSize: 10.5, fontWeight: 700, marginTop: 3, display: 'block' }}>
                                                                        $ {price ? calcPrice(d).toFixed(2) : '—'}
                                                                    </span>
                                                                </div>
                                                            )}
                                                            <div>
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, padding: 12, background: B.greenBg, borderRadius: 10, border: `1px solid ${B.greenBdr}`, marginBottom: 12 }}>
                                            {enabledSz.map(k => (
                                                <span key={k} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', background: B.green, color: '#FFFFFF', borderRadius: 99, fontSize: 11.5, fontWeight: 700 }}>
                                                    {k} · ${price ? calcPrice(stdSizes[k]).toFixed(0) : (stdSizes[k].customPrice || '—')} · {stdSizes[k].stock}pcs
                                                </span>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                        {[
                                            ['S/M/L/XL', () => { setEnabledSz(['S', 'M', 'L', 'XL']); toast.success('S/M/L/XL selected!'); }],
                                            ['All Sizes', () => { setEnabledSz(Object.keys(stdSizes)); toast.success('All sizes selected!'); }],
                                            ['Clear', () => setEnabledSz([])],
                                        ].map(([lbl, action]) => (
                                ...
                                <>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 12, padding: 16, background: B.surface, borderRadius: 11, border: `1px solid ${B.border}`, marginBottom: 14 }}>
                                        <Field label="Size Label">
                                            <input style={{ ...inpStyle(), width: 100 }} type="text" placeholder="14x14" value={niSize} onChange={e => setNiSize(e.target.value)} onFocus={focG} onBlur={blrB()} />
                                        </Field>
                                        <Field label="Stock">
                                            <input style={{ ...inpStyle(), width: 80 }} type="number" min="0" value={niStock} onChange={e => setNiStock(parseInt(e.target.value) || 0)} onFocus={focG} onBlur={blrB()} />
                                        </Field>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                                            {inchSizes.map((s, i) => (
                                                <div key={i} style={{ background: B.surfaceCard, border: `1px solid ${B.border}`, borderRadius: 11, padding: 14 }}>
                                                    ...
                                                    <div>
                                                        <p style={{ fontSize: 10, color: B.navyGhost, fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>Stock</p>
                                                        <input type="number" min="0" value={s.stock} onChange={e => edInch(i, 'stock', e.target.value)} style={{ ...inpStyle(), padding: '6px 10px', fontSize: 12 }} onFocus={focG} onBlur={blrB()} />
                                                    </div>
                                                    <span style={{ color: B.emerald.text, fontSize: 10.5, fontWeight: 700, marginTop: 5, display: 'block' }}>
                                                        $ {price && +price > 0 ? ((+price) * s.multiplier).toFixed(2) : '—'}
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        ...
                        <div style={{ ...cardStyle({ marginBottom: 0 }), padding: 18 }}>
                            ...
                            <Field label="Discount (%)" hint="Optional — percentage off, e.g. 10 = 10% off">
                                <input style={inpStyle()} type="number" placeholder="e.g. 10" min="0" max="99" value={discPrice} onChange={e => setDiscPrice(e.target.value)} onFocus={focG} onBlur={blrB()} />
                            </Field>
                            {discount && (
                        ...
                        <div style={{ background: B.goldBg, border: `1px solid ${B.goldBdr}`, borderRadius: 14, padding: 18 }}>
                            ...
                            <ul style={{ paddingLeft: 0, margin: 0, listStyle: 'none' }}>
                                {[
                                    ...
                                    'Item Details are shown on the product detail page',
                                ].map((tip, i) => (
