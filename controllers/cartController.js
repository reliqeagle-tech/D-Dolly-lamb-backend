...
//         const { itemId, size, color, quantity = 1, customPrice = 0 } = req.body;
...
const addToCart = async (req, res) => {
    try {
        ...
        // ✅ FIX - color is object array {name, hex} - check by name
        const colorExists = product.color.some(
        ...
        if (cartData[itemId][variantKey]) {
            ...
            cartData[itemId][variantKey] = { quantity, customPrice };
        }

        // ✅ FIX - markModified so Mongoose tracks nested object change
    ...
    try {
        ...
        if (quantity <= 0) {
            ...
            if (typeof existing === 'object') {
                ...
                cartData[itemId][variantKey] = { quantity, customPrice: 0 };
            }
        }
...
const getUserCart = async (req, res) => {
    try {
        ...
        let cartData = userData.cartData || {};
        console.log("Raw cartData from DB:", JSON.stringify(cartData)); // 🔍 Debug
