...
// /*
//   ═══════════════════════════════════════════════
//   ACTIVE SLIDER — LIGHT MODE (perf-optimized)
//   - Swiper lazy-mounts via IntersectionObserver (defers ~900ms reflow cost off initial load)
//   - Coverflow effect lightened (lower depth/rotate = fewer layout calcs)
//   - Hover box-shadow/border-color moved to a pseudo-element opacity fade
//     so only `transform` (composited) runs on the actual hover, not paint-heavy properties
//   ═══════════════════════════════════════════════
// */
...
//   /* ── Lazy-mount Swiper only when the section is near the viewport.
//      Defers Swiper's init cost (DOM measurement, coverflow transform calc,
//      loop-clone creation — the ~900ms forced reflow Lighthouse flagged)
//      off the critical initial-load path. rootMargin starts loading it
//      a bit before it's actually visible so there's no visible pop-in. ── */
//   useEffect(() => {
...
//         /* Active card highlight — border/shadow now live on ::after (opacity fade),
//            so only the border color driven here stays as a plain (cheap, one-time) swap
//            rather than an animated paint on every frame. */
//         .asl-swiper .swiper-slide-active .asl-card {
//           border-color: rgba(99,102,241,0.45) !important;
//         }
//         .asl-swiper .swiper-slide-active .asl-card::after { opacity: 1 !important; }
//         .asl-swiper .swiper-slide-active .asl-top-bar { width: 100% !important; }
//         .asl-swiper .swiper-slide-active .asl-tag { opacity:1 !important; }

//         /* ── Card — ONLY transform animates on hover now (composited, GPU) ── */
//         .asl-card {
//           transition: transform 0.38s cubic-bezier(0.16,1,0.3,1), border-color 0.38s ease;
...
//         .asl-card:hover .asl-top-bar { width: 100%; }
//         .asl-card:hover .asl-tag { opacity: 1; }
//         .asl-card:hover::after { opacity: 1; }
//         .asl-card:active { transform: translateY(-3px) scale(0.99) !important; }

//         /* ── Shadow simulated via pseudo-element opacity fade instead of
//            animating box-shadow directly — opacity is composited, box-shadow is not. ── */
//         .asl-card::after {
//           content: "";
//           position: absolute; inset: 0;
//           border-radius: 14px;
//           box-shadow: 0 20px 56px rgba(99,102,241,0.16), 0 4px 16px rgba(99,102,241,0.08);
//           opacity: 0;
...
//             grabCursor centeredSlides loop
//             autoplay={{ delay: 3800, disableOnInteraction: false, pauseOnMouseEnter: true }}
//             /* Lightened coverflow — lower depth/rotate/modifier means fewer
//                and cheaper per-slide transform calculations on every
//                slide-change and resize, while keeping the visual effect. */
...
//                     {/* Arrow CTA — kept as a CSS class instead of inline JS handlers
//                         so its hover is also just a class-driven transition, not a
//                         per-mouseenter style write */}
//                     <div className="asl-arrow-cta" style={{
//                       marginTop: "auto",
//                       width: 34, height: 34, borderRadius: "50%",
//                       display: "flex", alignItems: "center", justifyContent: "center",
//                       border: "1.5px solid rgba(99,102,241,0.28)",
...
//           </Swiper>
//         ) : (
//           /* Reserves the same height as the mounted Swiper so nothing
//              shifts (CLS-safe) while we wait for the section to scroll
//              into view. */
//           <div className="asl-swiper-placeholder" />
//         )}
//       </div>
...
import { useState, useRef, useEffect } from "react";
/* ── Swiper static imports poori tarah hata diye gaye hain.
   Ab ye sab neeche IntersectionObserver ke andar dynamically
   import() hote hain — sirf tab jab section viewport ke paas aaye.
   Isse Swiper ka poora JS+CSS weight Home page ke initial load se
   completely hat jaata hai. ── */

/*
  ═══════════════════════════════════════════════
  ACTIVE SLIDER — LIGHT MODE (perf-optimized)
  - Swiper lazy-mounts AND lazy-imports via IntersectionObserver
    (defers both the ~900ms reflow cost and the entire Swiper
    bundle weight off the critical initial-load path)
  - Coverflow effect lightened (lower depth/rotate = fewer layout calcs)
  - Hover box-shadow/border-color moved to a pseudo-element opacity fade
    so only `transform` (composited) runs on the actual hover, not paint-heavy properties
  ═══════════════════════════════════════════════
...
const ActiveSlider = () => {
  ...
  /* ── Dynamically-loaded Swiper pieces land here.
     Null until the IntersectionObserver fires and the
     import() promises resolve. ── */
  const [swiperModules, setSwiperModules] = useState(null);

  const swiperRef = useRef(null);
  const sectionRef = useRef(null);

  /* ── Lazy-mount AND lazy-import Swiper only when the section is
     near the viewport. Defers both Swiper's init cost (DOM
     measurement, coverflow transform calc, loop-clone creation —
     the ~900ms forced reflow Lighthouse flagged) AND its entire
     JS/CSS bundle off the critical initial-load path. rootMargin
     starts loading it a bit before it's actually visible so
        ...
        if (entry.isIntersecting) {
          Promise.all([
            ...
            setSwiperModules({
              ...
              EffectCoverflow: modulesMod.EffectCoverflow,
  ...
  return (
    ...
    >
      ...
      {/* ── Swiper (lazy-mounted + lazy-imported) ── */}
      <div style={{ padding: "0 16px", position: "relative", zIndex: 1 }}>
        {shouldRenderSwiper && swiperModules ? (
          <SwiperRenderer swiperModules={swiperModules} setActiveIndex={setActiveIndex} swiperRef={swiperRef} />
        ) : (
          <div className="asl-swiper-placeholder" />
        )}
      </div>
...
/* ── Separate component so the Swiper JSX only ever runs after
   swiperModules is guaranteed non-null — keeps the render logic
   readable instead of destructuring inline in the parent. ── */
const SwiperRenderer = ({ swiperModules, setActiveIndex, swiperRef }) => {
  const { Swiper, SwiperSlide, EffectCoverflow, Pagination, Autoplay } = swiperModules;
