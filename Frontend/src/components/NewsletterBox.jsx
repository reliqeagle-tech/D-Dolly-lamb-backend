...
const NewsletterBox = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    // ✅ FIX 2: Guard — if backendUrl is still somehow undefined, show clear error
    if (!backendUrl) {
      setErrorMsg("Server configuration error. Please try again later.");
      return;
    }
    ...
    try {
      ...
      // ✅ FIX 3: Check if response is ok before parsing JSON
      // Prevents "Unexpected end of JSON input" when server returns empty/HTML response
      if (!res.ok) {
    ...
    <section style={{
      ...
      <div className="nl-wrap relative z-10 flex flex-col items-center text-center px-6 py-16 md:py-20 max-w-2xl mx-auto">
        ...
        <h2 className="nl-title" style={{
          ...
          letterSpacing: "0.04em", lineHeight: 1.25, marginBottom: "14px",
        }}>
          Subscribe & receive{" "}
        ...
        {success ? (
          ...
          <div className="nl-form w-full max-w-md">
            ...
            {/* ✅ FIX 4: Show error inline instead of alert() */}
            {errorMsg && (
              <p style={{
                marginTop: "10px", fontSize: "11px", color: "#f87171",
                fontFamily: "Georgia, serif", fontStyle: "italic", letterSpacing: "0.04em",
              }}>
                ⚠ {errorMsg}
              </p>
            )}
